    ...
    def draw_lines_between_tracks(self, frames, specific_positions, transformed_positions, target_ids):
        ...
        # 현재 프레임 번호에 해당하는 프레임을 가져옴
            ...
            for id in target_ids:
                ...
                # 객체의 총 이동 거리와 속도를 가져옴
                dis = transformed_positions[frame_num][id]['total_distance']
                velo_ = transformed_positions[frame_num][id]['velocity']

                # 속도를 해당 객체의 기록에 추가
                speed_records[id].append(velo_)

                # 5프레임 평균 속도 계산
                avg_velo = sum(speed_records[id]) / len(speed_records[id])

                # 텍스트를 표시할 좌표를 계산
                coord = (frame_positions[id][0] + 40, frame_positions[id][1])
                coord2 = (frame_positions[id][0] + 40, frame_positions[id][1] + 20)

                # 프레임에 총 이동 거리와 평균 속도를 텍스트로 표시
                cv2.putText(frame, f"{dis:.2f} m", coord, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
                cv2.putText(frame, f"{avg_velo:.2f} km/h", coord2, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
